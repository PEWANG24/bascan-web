    ...
    <div className="min-h-screen bg-gradient-to-br from-green-600 via-green-700 to-green-800">
      ...
      <div className="max-w-7xl mx-auto py-4 sm:py-6 px-4 sm:px-6 lg:px-8">
        <div className="space-y-4 sm:space-y-6">
          ...
          {/* Recent Activations */}
          {activeTab === 'activation' && (
            <div className="bg-white shadow-xl rounded-xl border-l-4 border-green-500">
              <div className="px-4 py-5 sm:p-6">
                <h3 className="text-lg leading-6 font-black text-green-800 mb-4 flex items-center space-x-2">
                  <span>📊</span>
                  <span>Recent SIM Activations</span>
                </h3>
                
                {activations.length === 0 ? (
                  <p className="text-gray-500 text-sm text-center py-8">No activations submitted yet.</p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-green-50">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-black text-green-800 uppercase tracking-wider">
                            Serial Number
                          </th>
                          <th className="px-6 py-3 text-left text-xs font-black text-green-800 uppercase tracking-wider">
                            Market Area
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {activations.slice(0, 10).map((activation) => (
                          <tr 
                            ...
                            onClick={() => {
                              setSerialNumber(activation.serialNumber);
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                              {activation.marketArea}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                activation.reviewStatus === 'approved' 
                                  ? 'bg-green-100 text-green-800'
                                  : activation.reviewStatus === 'rejected'
                                  ? 'bg-red-100 text-red-800'
                                  : 'bg-yellow-100 text-yellow-800'
                              }`}>
                                {activation.reviewStatus}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {new Date(activation.timestamp).toLocaleDateString()}
                            </td>
